...
interface AddPatientAvailabilityFormProps {
	...
	onAddPatientAvailability: (patient: PatientData, scheduleBlocks: ScheduleBlockData[]) => void;
}

const days = ['M', 'T', 'W', 'Th', 'F'];

const AddPatientAvailabilityForm: FC<AddPatientAvailabilityFormProps> = ({ scheduleBlocks = [], patient, onAddPatientAvailability }) => {
	const [selectedBlocks, setSelectedBlocks] = useState<{
		[key: string]: number | null;
	}>({});

	const blocksByDay = scheduleBlocks.reduce((accum: { [key: string]: ScheduleBlockData[] }, block) => {
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
		...
		const scheduleBlockIds = Object.values(selectedBlocks).filter((b): b is number => b !== null);
		if (scheduleBlockIds.length > 0) {
			const patientAvailablities = await putPatientAvailabilty(patient.id, scheduleBlockIds);
			onAddPatientAvailability(patient, patientAvailablities);
		}
	};

	const handleChange = (day: string) => (event: ChangeEvent<HTMLSelectElement>) => {
		const selectedBlock = scheduleBlocks.find((block) => String(block.id) === event.target.value);
		setSelectedBlocks({
			...selectedBlocks,
